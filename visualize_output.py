...
parser.add_argument("--curJob", type=int, default=1, help="split the moment for this annotator.")
...
if __name__ == "__main__":
  ...
  count = 0
  prediction_dict = {k: int(k.split('_')[-2]) for k in prediction.keys()}
  prediction_dict = {k: v for k, v in sorted(prediction_dict.items(), key=lambda item: item[1])}
  for traj_id in tqdm(prediction_dict):
    count += 1
    ...
    video_file = os.path.join(args.multivideo_path, "%s.mp4" % '_'.join(traj_id.split('_')[:-2]))

    target_path = os.path.join(args.vis_path)
    ...
    while cur_frame < frame_count:
      ...
      for future_id in gt:
        if args.show_obs:
          frame_data = plot_traj(frame_data, [one[2:] for one in gt[future_id]["obs_traj"]], (0, 255, 255))     
      target_file = os.path.join(args.vis_path, "%08d.jpg" % (count-1))
