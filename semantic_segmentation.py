...
if __name__ == "__main__":
  ...
  imgs = [one.strip() for one in open(args.imglst, "r").readlines()][::args.every]

  tfconfig = tf.ConfigProto()
  tfconfig.gpu_options.allow_growth = True
  tfconfig.gpu_options.visible_device_list = "%s" % (",".join(["%s" % i for i in [args.gpuid]]))
  ...
  with graph.as_default():
    with tf.Session(graph=graph, config=tfconfig) as sess:
      ...
      for img in tqdm(imgs):
        ...
        video_name = '_'.join(os.path.splitext(os.path.basename(img))[0].split('_')[:-2])
        ...
        if not os.path.exists(os.path.join(args.out_path, video_name)):
          os.makedirs(os.path.join(args.out_path, video_name))
        targetfile = os.path.join(args.out_path, video_name, "%s.npy" % imgname)
